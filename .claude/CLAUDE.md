...
- **Transparent PNG stamp paints with the PNG's stored RGB under transparent pixels (typically black)** *(fixed)*: `create_image_xobject()` used to call `to_rgb8()` and drop the alpha channel; PNGs commonly store `RGB=(0,0,0)` under fully-transparent pixels, so the stamp rendered with a black halo. Now decodes RGBA when the source has alpha, splits into a straight-RGB main image plus a single-channel DeviceGray Image XObject referenced via `/SMask`. Covered by `stamp_image_transparent_png_does_not_paint_black_background` (pdfium pixel probe) and a structural assertion `assert_stamped_pdf_has_alpha_smask` that catches "alpha was dropped again" regressions at the PDF-object level without needing to render. Opaque sources verified to skip SMask via `stamp_image_opaque_png_does_not_emit_smask`.
